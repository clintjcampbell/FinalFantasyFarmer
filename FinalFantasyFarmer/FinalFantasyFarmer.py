...
libname = "C:/Program Files (x86)/Tesseract-OCR/libtesseract-4.dll"
...
def checkSecretGift():
    clickPoint(114,646)
    time.sleep(.5)
    collect, loc , max = checkTemplate((135,509,334,567),CollectTemp, .75)
...
def checkforGold():
    ...
    if More:
        ...
        #time.sleep(1.5)
        Treasury, loc , max = checkTemplate((23,571,134,691),TreasuryTemp, .8)
        if Treasury:
            clickPoint(62, 619)
            time.sleep(1)
        ...
        for j in range(3):
            for i in top_windows:
                if 'BlueStacks' in i[1]:
                    ...
                    collect, loc, max = checkTemplate((43,744,458,852),CollectTemp,.7)
                    if collect:
                        clickPoint(236, 787)
                    CheckEmpireScreen()
                    CheckFreeBuild()
